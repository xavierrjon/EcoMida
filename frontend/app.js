class EcoMidaApp {
  ...
  async loadNotificationModule() {
    ...
    try {
      ...
      if (
        ...
        typeof window.notificationSettings.showSettings !== "function"
      ) {
        await this.loadScript("notification-settings.js");
      }

      if (
        !window.notificationsSimple ||
        typeof window.notificationsSimple.setup !== "function"
      ) {
      ...
      if (
        window.notificationSettings &&
        typeof window.notificationSettings.init === "function"
      ) {
  ...
  setupEventListeners() {
    document.addEventListener("click", async (e) => {
      ...
      if (
        e.target.id === "notification-settings-btn" ||
        e.target.closest("#notification-settings-btn")
      ) {
        if (!this.currentUser) {
        ...
        try {
          ...
          if (
            ...
            this.showNotification(
              "Configurações de notificação não disponíveis",
              "warning",
            );
          }
        } catch (error) {
      ...
      if (!this.notificationModuleLoaded) {
        setTimeout(async () => {
          try {
            ...
            if (
              window.notificationsSimple &&
              typeof window.notificationsSimple.showAlertsInUI === "function"
            ) {
              setTimeout(() => {
                window.notificationsSimple.showAlertsInUI();
              }, 2000);
            ...
            console.warn(
              "Notificacoes carregadas em background com erro:",
              error,
            );
          }
        }, 1000);
      ...
      setTimeout(() => {
        ...
        setTimeout(async () => {
          try {
            ...
            console.error(
              "❌ Erro inicial no carregamento de notificações:",
              error,
            );
  ...
  async handleAddFood(e) {
    ...
    if (!name || !expiry_date) {
      this.showNotification(
        "Nome e data de validade são obrigatórios",
        "error",
      );
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        this.showNotification(
          data.error || "Erro ao cadastrar alimento",
          "error",
        );
      }
    } catch (error) {
